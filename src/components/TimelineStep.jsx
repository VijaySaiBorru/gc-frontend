...
const TimelineStep = ({stepIndex,order,step,isCompleted,isCurrent,isLastStep,icon,description}) => {
  ...
  return (
    <li className='relative mb-6 sm:mb-0 sm:pl-10'>
        ...
        <div className={`z-10 flex items-center justify-center w-6 h-6  ${step?.status === 'completed' ? 'bg-green-900 text-green-100' : step?.status === 'shipped' ? 'bg-indigo-900 text-indigo-100' : step?.status === 'processing' ? 'bg-blue-600 text-blue-100' : 'bg-red-700 text-red-100'}  
        ${iconBgColor} 
